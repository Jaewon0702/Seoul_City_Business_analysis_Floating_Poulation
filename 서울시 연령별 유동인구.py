...
del df['일자'], df['시간'], df['성별'], df['시']  # 필요없는 열은 삭제
...
age_max=71        #20대~70대까지 분석

for i in range(20,age_max,10) :                      #각 연령별 유동인구의 합
    df3['%d대'%(i)]=np.sum(df3[i],axis=1)
    del df3[i]
